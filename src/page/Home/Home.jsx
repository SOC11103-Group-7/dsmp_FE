      ...
      {profile ? (
        <div className='flex flex-col w-full md:w-1/2 md:px-10 px-5'>
          <div className='w-full overflow-hidden mt-6'>
            <textarea type='text' className='w-full h-full rounded-lg p-4 font-bold text-sm text-gray-900 bg-transparent outline-none' placeholder="What's on your mind??" onChange={(e) => setPost(e.target.value)} />
          </div>
              ...
              <>
                {posts?.map((post, index) => (
                  <div key={index} className='w-full bg-box border border-black rounded-lg overflow-hidden mt-4 '>
                    <div className='ml-2 mt-2'>
                      <div className='flex items-center'>
                        <div className='w-10 h-10 bg-cover bg-center bg-no-repeat rounded-full' style={{ backgroundImage: `url(${post.author.avatar})` }}> </div>
                        <div className='ml-2'>
                    ...
                    <div className='flex justify-end'>
                      <div className="flex flex-col place-items-center">
                        <span className=' block p-2 text-sm font-bold text-gray-900'>{ethers.utils.formatEther(post.tipsAmount)} ETH</span>
                        {address = post.author.address || !profile ? null :
                          <button className='w-20 h-8 m-2 bg-gray-300 text-gray-900 font-semibold text-xs rounded-full border border-black'>Tip 0.1 ETH</button>
                        }
                      </div>
                    </div>
                  </div>
